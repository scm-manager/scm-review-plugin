...
const CommentActionToolbar: FC<Props> = ({comment, createLink, collapsed, ...actions}) => {
  ...
  if (hasLink("delete")) {
    icons.push(
      <ToolbarIcon title={t("scm-review-plugin.comment.delete")} icon={"trash"} onClick={actions.onDelete}/>
    );
  }

  if (hasLink("update")) {
    icons.push(
      <ToolbarIcon title={t("scm-review-plugin.comment.update")} icon={"edit"} onClick={actions.onUpdate}/>
    );
  }

  if (hasLink("reply")) {
    icons.push(
      <ToolbarIcon title={t("scm-review-plugin.comment.reply")} icon={"reply"} onClick={actions.onReply}/>
    );
  }

  if (hasTransition("SET_DONE")) {
    icons.push(
      <ToolbarIcon title={t("scm-review-plugin.comment.done")} icon={"check-circle"}
  ...
  if (hasTransition("MAKE_TASK")) {
    icons.push(
      <ToolbarIcon title={t("scm-review-plugin.comment.makeTask")} icon={"tasks"}
  ...
  if (hasTransition("REOPEN")) {
    icons.push(
      <ToolbarIcon title={t("scm-review-plugin.comment.reopen")} icon={"undo"}
  ...
  if (hasTransition("MAKE_COMMENT")) {
    icons.push(
      <ToolbarIcon title={t("scm-review-plugin.comment.makeComment")} icon={"comment-dots"}
